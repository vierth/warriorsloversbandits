    ...
    if (drawData){
        ...
        if (data[i].size > 0) {
            ...
            if (activeCategories.has(data[i][activecatnum])){
                if (!filteredTexts||(filteredTexts && filteredTexts.includes(data[i][0].split("_")[0])))
                {
...
legend.append("text")
    ...
    d3.select("#search").on("keypress", function(){
        if (d3.event.keyCode === 13) {
            ...
            filteredTexts = this.value.replace(/\s/g, '').replace(/，/g, ",").split(",")
            
            if (filteredTexts.length == 1 && filteredTexts[0].length == 0){
                filteredTexts = false
            }
